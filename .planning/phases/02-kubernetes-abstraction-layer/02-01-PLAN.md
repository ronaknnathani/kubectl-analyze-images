...
**IMPORTANT: runtime.Object compatibility.** `*corev1.PodList` and `*corev1.NodeList` both implement `runtime.Object` (via embedded `metav1.TypeMeta`). This means the pager closure `func(ctx, opts) (runtime.Object, error)` can call `c.k8sClient.ListPods(ctx, namespace, opts)` directly — the return type `*corev1.PodList` satisfies `runtime.Object` without any casting. The same applies to `ListNodes` returning `*corev1.NodeList`. This is the key reason the interface methods return typed list pointers rather than `runtime.Object`.
