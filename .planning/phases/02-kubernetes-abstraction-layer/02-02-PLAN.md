...
must_haves:
  truths:
    ...
    - "Test coverage for internal/cluster and internal/analyzer packages exceeds 60%"
  artifacts:
...
4. Refactor `ListPods` method:
   ...
   - Change the pager function from `c.clientset.CoreV1().Pods(namespace).List(ctx, opts)` to `c.k8sClient.ListPods(ctx, namespace, opts)` — note: the pager passes its own opts, so construct the initial `listOptions` with `ResourceVersion: "0"` and `LabelSelector`, then the pager callback calls `c.k8sClient.ListPods(ctx, namespace, opts)` where `opts` is the pager-provided opts
...
5. Refactor `GetImageSizesFromNodes` method:
   ...
   - Change the pager function from `c.clientset.CoreV1().Nodes().List(ctx, opts)` to `c.k8sClient.ListNodes(ctx, opts)`
...
1. Add import: `"github.com/ronaknnathani/kubectl-analyze-images/pkg/kubernetes"` and `"github.com/ronaknnathani/kubectl-analyze-images/internal/cluster"`.
...
2. In `runAnalyze` function, change the analyzer creation sequence:
   ...
   - TO:
     ...
     ```
   - Update the variable name from `analyzer` to `podAnalyzer` (to avoid conflict with the package name `analyzer`).
   - Update `analyzer.AnalyzePods(...)` to `podAnalyzer.AnalyzePods(...)`.

3. Verify the import for `"github.com/ronaknnathani/kubectl-analyze-images/internal/cluster"` is added.
  </action>
...
**IMPORTANT:** The ListPods and GetImageSizesFromNodes methods use spinners that write to stderr. The tests will still work because the spinner writes don't affect return values. If stderr output is a concern, the tests can redirect os.Stderr temporarily, but this is NOT required — just let it write to stderr during tests.
